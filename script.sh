...
change_server_name() {
    ...
    echo "🏷️  Server name changed to: $NEW_SERVER_NAME"
}
...
delete_all_channels() {
    ...
    echo "⚠️  WARNING: About to delete ALL channels!"
    ...
    if [ "$confirm" = "YES" ]; then
        ...
        for channel_id in $channels; do
            curl -X DELETE "https://discord.com/api/v10/channels/$channel_id" \
                -H "Authorization: Bot $DISCORD_TOKEN" \
                -s
            echo "🗑️  Deleted channel: $channel_id"
        done
        
        echo "💥 All channels deleted!"
    else
...
while read -p "> " user_input; do
    if [ "$user_input" = "botgetgone" ]; then
        if [ ! -z "$SERVER_ID" ]; then
            ...
            echo "❌ No server loaded. Paste an invite code first."
        fi
        ...
        echo "❌ Invalid input. Paste an invite code or type 'botgetgone'."
    fi
